  (function (){
    ...
    deviceWH = function (arg) { let w,h; 
      ...
      resize: function (cb, capt){
        evtListener(win, 'resize', function (){
          cb.call($$,deviceWH('w'),deviceWH('h'));
        }, capt);
        return $$.device;
      },
    ...
    INIT_MIRROR = {
      cssMedia: {
        tablet: s => `@media only screen and (min-width: 600px){${s}}`,
        landscape: s => `@media screen and (orientation:landscape){${s}}`,
        portrait: s => `@media screen and (orientation:portrait){${s}}`,
        portraitFullscreen: s => `@media screen and (orientation:portrait) and (display-mode:fullscreen){${s}}`,
        landscapeFullscreen: s => `@media screen and (orientation:landscape) and (display-mode:fullscreen){${s}}`,
        fullscreen: s => `@media screen and (display-mode:fullscreen){${s}}`,
      },
      each: function() {
        let a = arguments, aL = $$.len(a), a1 = a[0],
        a2 = a[1], a3 = a[2], t1 = $$.typo(a1),
        t2 = $$.typo(a2), t3 = $$.typo(a3);
        if ((aL == 2) && ($$.isArr(a1) && t2 == $fn)) {
          for (let i = 0; i < $$.len(a1); i++){
            a2.call($$, a1[i], i, a1)
          }
        }
          ...
          let z = a1.split(a2);
          for (let j = 0; j < $$.len(z); j++){
          a3.call($$, z[j], j, z) } }
          else if (aL == 2 && ((t1 == $str && a1.indexOf("-") > 0) && t2 == $fn)) {
     
          let n = a1.split('-'),
            n1 = $$.pF(n[0]), 
            n2 = $$.pF(n[1]);
          if (n1 > n2) { for (let y = n1; y > n2; y--) { a2.call($$, y, (n1 - n2)) } } else { for (let l = n1; l < n2; l++) { a2.call($$, l, (n2 - n1)) } }
        } else { return $$ }
      },
      guid: (L, n) => {
      let c = `${$$.alpha()}${n === $t ? $$.int() : ''}${$$.alpha($t)}`,
       buf = [],bufx = [], d = 32, l = $$.len(c);
        L = L === $t ? $$.randNum(4,33) : (typo(L) == $num ? L : d);
       $$.each(`0-${L}`, i => { 
         buf[i] = c.charAt(Math.floor(Math.random() * l ))
        });
        $$.str.times = x => {
          x=x||1
       $$.each(`0-${x}`, i => { 
      ...
      runJs: function(a, p){
        ...
        let arr = $.map(a, scr => {
          return $.getScript( (p ||"")+scr+'.js');
        });
        arr.push($.Deferred( df => { $( df.resolve ) } ));
      ...
      smoothScroll: function (el){
        q(el).scrollIntoView({behavior: 'smooth'});
        return this;
      },
      ...
      tab: function(sel, indx, act, c, c1){
        ...
        sel = qa(`.${sel.trim()}`);
        $$.each(`0-${$$.len(sel)}`, i => {
          sel[i].className = sel[i].className.replace(` ${act}`, '');
        });
      ...
      tag: function (o = {}){
        if(typo(o) == $obj){
          ...
          eTag = t => `${ o.text || '' }</${elem}>`;
          return (!selfClose.includes(elem)) ? sTag()+eTag() : sTag($t);
        }else{ return $$ }
      },
      alpha: function() {
        let a = arguments,
      ...
      addJs: function(s,b){ 
        s = `${s}.js?_=${$$.guid(true,true)}`; 
        let script = doc.createElement('script');
        script.src = s; 
        script.type = 'text/javascript';
        b === $t ?doc.body.append(script):doc.head.append(script)
      },
      addCss: function(s,id,b){ 
        s = `${s}.css?_=${$$.guid()}`; 
        ...
        style.href = s; 
        style.type = 'text/css';
        b === $t ?doc.body.append(style):doc.head.append(style)
      }
    },//INIT_MIRROR
     ...
     STR_PROTOTYPE = {
      ...
      getPos: function(s, ca){
        if (s == "*" || s == '.' || s == "+") { s = "\\" + c }
        let m_arr = [...this.matchAll(new RegExp(`${s}`,`g${ ca === true ? '' : 'i'}`))];
        st = m_arr.map( m => m.index );
        e = st.map(i => i + $$.len(s));
        $$.extend.arr({start: st, end: e, len: $$.len(st) });
        return st;
      },
